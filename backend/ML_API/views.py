...
def getImages(name=None, cat=None, classNum = 1):
     ...
     for img in imgs:
          try:
               ...
               continue
          image = [cv2.cvtColor(np.array(image), cv2.COLOR_BGR2RGB), 0 if cat == "Default" else classNum]
...
def loadData(links, cat_name="Default", save=False):
     ...
     for link in links:
          ...
          imgs.append([cv2.cvtColor(np.array(img), cv2.COLOR_BGR2RGB), 0 if cat_name == "Default" else 1])
          if(save):
...
def predict(request):
     ...
     if(pred_request_serializer.is_valid()):
          ...
          if(url is not None and len(url) > 0):
               image = cv2.cvtColor(skio.imread(url), cv2.COLOR_BGR2RGB)
